...
class HBot:
    ...
    def responde(self, texto, estado_atual):
        ...
        if eh_valida:
            ...
            # Se o status atual for final, pula os blocos de execução de fluxo normal e seta a resposta
    ...
    def encontra_resposta(self, status):
        ...
        if status == '0':
            ...
            table = self.commands.select_table(status)

            if table == 'tabela inválida':
                resposta = 'Não existem tabelas de informação para a sua pergunta, por favor digite *0* para voltar ao Menu inicial e notifique o administrador'
                return resposta, status

            else:


                resposta, eh_final = self.commands.select_response(status, table)

                if resposta == 'invalida':
                   self.imprime_resposta('Não possuo resposta cadastrada para o índice:' + status)
                   eh_final = '0'

                else:
                    # Valida se é um status final, seja
                    if eh_final == '1' or eh_final == '2':
    ...
    #Só entra aqui se o texto não for VOLTAR
